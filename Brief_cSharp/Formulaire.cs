        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (errors.Count == 0 && txtnom.Text != "" && txtPrenom.Text != "" && txtEmail.Text != "" && txtDate.Text != "" && txtTel.Text != "")
            {
                ...
                try
                {
                    ...
                    string req = "insert into apprenant (nom,prenom,email,telephone,adresse,pays,ville,choix,date_naissance) values ('" + txtnom.Text + "','" + txtPrenom.Text + "','" + txtEmail.Text + "','" + txtTel.Text + "','"  + txtADR.Text + "','"+ cmbPays.Text + "','" + cmbVille.Text + "','" + cmbSpecialite.Text + "','" + CorrectDateFormat + "')";
                    SqlCommand cmd = new SqlCommand(req, cn);
                    
                    if(cmd.ExecuteNonQuery().Equals(1))
                    {
                        MessageBox.Show("L'ajout effectué avec succée");
                    }
                    else
                    {
                        MessageBox.Show("L'echec d'ajout");
                    }
